...
export default function App({ Component, pageProps }) {
  ...
  const [newsletterContentContext, setNewsletterContentContext] = useState({
    heading: 'The latest from the JBS crew, directly to your inbox.',
    text: 'Once a month updates with no spam. New projects, behind-the-scenes stuff, and opportunities, all in one place',
    image: null
  });
    ...
    <ReactLenis root options={{ lerp: 0.125 }}>
      ...
      <IntroContext.Provider value={[introContext, setIntroContext]}>
        <ReelContext.Provider value={[reelContext, setReelContext]}>
          <NewsletterContentContext.Provider value={[newsletterContentContext, setNewsletterContentContext]}>
                      ...
                      >
                        ...
                        <div className="flex flex-wrap h-full">
                          <div className="w-full md:w-1/2 h-[40%] md:h-full relative overflow-hidden">
                            {newsletterContentContext.image ? (
                              <Image
                                layout={'fill'}
                                image={newsletterContentContext.image}
                                widthOverride={900}
                                className={'w-full h-full absolute inset-0 object-cover object-top md:object-center scale-[1.0025] force-top'}
                              />
                            ) : (
                          ...
                          <div className="w-full md:w-1/2 h-[60%] md:h-full p-5 md:p-10 flex flex-col items-center justify-center relative">
                            ...
                            <div className="w-full">
                              ...
                              <h2 className="text-[6.5vw] md:text-[3.55vw] leading-[0.88] md:leading-[0.88] text-center mb-4 md:mb-6">{newsletterContentContext.heading}</h2>
                              <span className="block leading-none text-xs md:text-sm md:leading-none font-mono mx-auto text-center mb-2 md:mb-3 lg:mb-3 max-w-[600px] w-full md:w-10/12">{newsletterContentContext.text}</span>
            ...
            </NewsletterContentContext.Provider>
