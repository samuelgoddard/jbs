...
export default function WorkCarousel({ work }) {
  ...
  return (
    <>
    <div className="fixed top-0 left-0 w-[85vw] md:w-[66.75vw] z-20" ref={box}>
    ...
    <div className="grid grid-cols-9" id="new-container">
      <div className="col-span-9 md:col-span-3 md:col-start-7 flex justify-center p-3">
        <div className="w-full md:w-9/12">
          ...
          {[...Array(5)].map((index) => ( 
            <div from={{ y: 35, opacity: 0 }} key={index}>
              {work.map((e, i) => {
                ...
                return (e.category == currentCat || currentCat == 'all') & (e.type == currentType || currentType == 'all') ? (
                  <li className={`block mb-8 md:mb-16 pt-3 ${width}`} key={i} onMouse={() => setCurrent(i)}>
