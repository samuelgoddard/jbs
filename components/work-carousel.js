              ...
              <div key={index}>
                {work.map((e, i) => {
                  ...
                  return (e.category == currentCat || currentCat == 'all') & (e.type == currentType || currentType == 'all') ? (
                    <li className={`block mb-8 md:mb-[7vw] transition-all ease-in-out duration-300  ${width} ${current == i ? 'opacity-100 border p-4 border-black border-opacity-25' : 'opacity-50 border-opacity-0 grayscale p-0'}`} key={i} data-scroll data-scroll-repeat data-scroll-call={i} data-scroll-offset="60%, 40%" onClick={() => setCurrent(i)}>
