    ...
    <header className="font-mono w-full absolute top-0 left-0 right-0 p-3 z-[100000000]">
      ...
      <div className="flex flex-wrap relative">
        <Link scroll={false} legacyBehavior href="/">
          <a className={`mb-1 md:mb-0 block w-[60px] overflow-hidden transition-all ease-in-out duration-300 fixed top-3 left-3 ${router.asPath == '/studio' ? 'text-white' : 'text-black' } ${ router.asPath == '/' || router.asPath.includes('/cat') ? 'opacity-100' : 'delay-[600ms]' } ${(router.asPath == '/' || router.asPath.includes('/cat')) && logoActive ? '!opacity-0 pointer-events-none' : '!opacity-100' }`}>
